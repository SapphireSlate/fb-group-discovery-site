...
export default async function TrendingPage() {
  ...
  // Fetch trending groups - sorted by highest view count
  const { data: trendingGroups, error } = await supabase
    .from('groups')
    .select(`
      *,
  ...
  if (trendingGroups && trendingGroups.length > 0) {
    // Get all group IDs
    const groupIds = trendingGroups.map(group => group.id);
    ...
    groupsWithTags = trendingGroups.map(group => ({
      ...group,
      ...
      <main className="flex-1">
        <div className="container mx-auto py-6 px-4">
          <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
            <div>
              <h1 className="text-3xl font-bold">Trending Facebook Groups</h1>
              <p className="text-muted-foreground mt-1">
                The most popular and active Facebook groups right now
              </p>
            </div>
          ...
          <div className="mt-8">
            {groupsWithTags && groupsWithTags.length > 0 ? (
              ...
              <div className="text-center py-12">
                <h3 className="text-lg font-medium">No trending groups found</h3>
                <p className="text-muted-foreground mt-2">
                  Check back later for trending groups or browse all groups.
                </p>
